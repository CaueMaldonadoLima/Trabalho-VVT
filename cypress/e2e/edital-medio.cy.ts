...
describe('Sistema Integrado de Gestão para Fundações de Amparo a Pesquisas', () => {
  ...
  it.only('Realiza login no sistema e cria um edital médio', () => { //Teste edital simples, se houver mais de um teste, o it.only executa apenas esse teste.
    ...
    cy.get('[data-cy="nome"]').type(
      'Grupo-08 E.M. 002/2025 vinicius-aguiar', //Edite essa linha para preencher o nome do Edital
      { delay: 0 },
  ...
  .click()
  .focus()
  .realType('Termo de Aceite Edital Medio - Grupo 8');



    cy.get('[data-cy="texto-do-edital"]').click();
    cy.get('[data-cy="texto"]').should('be.visible')
  .click()
  .focus().realType('Texto do Edital (Medio) - Grupo 8'); 
    ...
    cy.get('[data-cy="estado-sao-paulo"]').click();//Desmarca São Paulo que já vem marcado por padrão
